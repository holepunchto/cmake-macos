# cmake-macos
Functions for streamlining the creation of macOS application bundles (.app), including icon management, code signing, and more.
...
#### Options
##### `<target>` 
The name of the CMake target associated with the entitlements.
##### `DESTINATION <path>`
...
#### Options 
##### `<target>`
The name of the CMake target associated with the icon set creation.
##### `DESTINATION <path>`
...
#### Options
##### `<target>`
The name of the CMake target associated with the Info.plist generation.
##### `DESTINATION <path>`
Specifies the location to save the Info.plist file. Defaults to Info.plist within the build directory.
##### `NAME <string>`
...
##### `EXECUTABLE <path>`
Path to the application executable (if not providing the TARGET option).
...
#### Options
##### `<target>`
The name of the primary CMake target for the macOS application. This is used to link dependencies and create the overall bundle structure.
##### `DESTINATION <path>`
The desired output path for the generated application bundle. Defaults to <target_name>.app in the build directory.
##### `INFO <path>`
